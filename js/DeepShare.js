...
function DeepShare(app_id) {
    ...
    this.CreateWorker = function(params) {
        _createWorker(params, params);
    };
}
...
function DeepShareWorker(params) {
    ...
    var _dealIOS = function(tag, dst, url) {
        ...
        if ((_Params.ios_major <= 8) && !IsNullOrUndefined(url)) {
            _gotoUrl(url);
        } else {
            // gotoTip will test uni-link by default
            _gotoTip(tag, dst);
        }
    };

    var _dealTecent = function(dstIos, dstAndroid) {
        if (_Params.is_ios) {
            if ((_Params.ios_major < 9 && _Params.is_yyb_enable_ios_below_9) ||
                    (_Params.ios_major >= 9 && _Params.is_yyb_enable_ios_above_9)) {
                _gotoUrl(_Params.yyb_url);
            } else {
                _gotoTip('ios', dstIos);
            }
        } else if (_Params.is_android){
            if (_Params.is_yyb_enable_android) {
                _gotoUrl(_Params.yyb_url);
            } else {
                _gotoTip('android', dstAndroid);
            }
    ...
    this.Start = function() {
        ...
        */
        ...
        if (_Params.is_wechat) {
            ...
            _dealTecent(_DSAction.destination.dstweixintipios, _DSAction.destination.dstweixintipandroid);
            /*
            if (_shouldGotoYYB()) {
            ...
            */
            ...
            _dealTecent(_DSAction.destination.dstqqtipios, _DSAction.destination.dstqqtipandroid);
            /*
            if (_shouldGotoYYB()) {
                DeepShare.dsLogDebug('YYBurl is: ' + _Params.yyb_url);
    ...
    this.GetDsAction = function() {
        return _DSAction;
    };
    this.GetDstLocation = function() {
        return _dstLocation;
    };
    this.GetDeeplinkLocation = function() {
        return _deeplinkLocation;
    };
}
